 ...
 * @param {boolean}
 */
async function isFirstTimeMessage(senderFbUserId) {
 ...
 */
async function reply(toFbUserId, fromFbPageId, message, clientId, pageAccessToken) {
 ...
 * @return {string}
 */
function generateEmailHtmlOfProduct(product) {
    return `<table>
                 <tr>
 ...
 */
export async function handleFbMessage(clientId, fbMessageObj) {
    let replyMessage = '';
    ...
    if (!senderFbUserId || !fbPageId || !text)
        console.error("Missing data from Facebook webhook's message object");
    ...
    if (!pageAccessToken)
        throw new Error('Cannot find page access token.');
    ...
    if (isFirstTime) {
        ...
        // todo: overwrite the value for testing purpose. Remove this line
        text = '/buy 312290';
        ...
        if (found) {
            ...
            if (['/desc', '/price', '/shipping', '/buy'].includes(action)) {
                ...
                if (matchedProduct) {
                    if (action === '/desc')
                        replyMessage = matchedProduct.description;
                    else if (action === '/price')
                        replyMessage = `${matchedProduct.price}`;
                    else if (action === '/shipping')
                        replyMessage = `${matchedProduct.shipping}`;
                    else {
                        const emailData = {
                            to: email,
                    ...
                    replyMessage = 'The product ID is invalid.';
            }
            else {
