...
import {FbMessage, handleFbMessage} from "../controllers/customer.controller.js";
import {returnError} from "../middlewares/error.js";

const router = Router();

router.get("/messaging/:clientId", (req: Request, res: Response, next: NextFunction) => {
    try {
...
router.post("/messaging/:clientId", async (req: Request, res: Response, next: NextFunction) => {
    const clientId = req.params.clientId;
    let body = req.body;
    let fbMessage: FbMessage | undefined;
    try {
        // Checks this is an event from a page subscription
        if (body.object === 'page') {

            // Iterates over each entry - there may be multiple if batched
            body.entry.forEach((entry: { messaging: FbMessage[]; }) => {
            ...
            if (clientId && fbMessage) await handleFbMessage(clientId, fbMessage);
            else {
